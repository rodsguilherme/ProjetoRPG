<template>
  <v-sheet class="mx-auto" elevation="8" max-width="100%" height="100%">
    <v-slide-group v-model="model" class="pa-4" mandatory show-arrows>
      <v-slide-item v-for="(race, i) in races" :key="i"  :id="race.id" v-slot:default="{ active, toggle }">
        <v-card
          class="ma-4"
          :color="blue"
          height="400"
          width="280"
          @click="{raceSelected(race.id, race.name)}"
        >
...
export default {
  ...
  methods: {
    raceSelected(id,name) {
      this.idRace = id
      this.nameRace = name
      window.alert(`${this.nameRace}, ${this.idRace}`)
    }
  }
};
</script>